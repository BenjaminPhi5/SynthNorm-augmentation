...
def simple_augmentations(image_keys=['FLAIR'], label_keys=['label', 'brainmask'], out_spatial_dims=(80, 192, 160)):
    ...
    return Compose([
        resizer,
        AffineAugment_V2(p=0.2, spatial_size=out_spatial_dims, dims=dims, axial_only=True, allow_translate=False, allow_shear=False, keys=combined_keys, mode=['bilinear' for _ in range(len(image_keys))] + ['nearest' for _ in range(len(label_keys))]),
...
def simple_augmentations_with_biasfield(image_keys=['FLAIR'], label_keys=['label', 'brainmask'], out_spatial_dims=(80, 192, 160), bias_field='perlin'):
    combined_keys = image_keys + label_keys
    resizer = MonaiCropAndPadToShape3d_V2(out_spatial_dims, keys=combined_keys)
    dims = 3
    ...
    return Compose([
        resizer,
        AffineAugment_V2(p=0.2, spatial_size=out_spatial_dims, dims=dims, axial_only=True, allow_translate=False, allow_shear=False, keys=combined_keys, mode=['bilinear' for _ in range(len(image_keys))] + ['nearest' for _ in range(len(label_keys))]),
        ArtefactsAugmentation(p_bias_field=0.5, p_downsample=0, p_motion=0., p_ghosting=0.0, p_spike=0.0, modality_p=1.0, bias_field=bias_field),
        GaussianNoiseAugment(p=0.15, dims=dims, keys=image_keys),
        BrightnessAugment(p=0.15, dims=dims, keys=image_keys),
        ContrastAugment(p=0.15, dims=dims, keys=image_keys),
        MirrorAugment(p=0.5, dims=dims, keys=combined_keys),
...
def nnunet_augmentations(image_keys=['FLAIR'], label_keys=['label', 'brainmask'], out_spatial_dims=(80, 192, 160), add_synthetic=False, synthetic_realistic=True, use_format_transform=True):
    combined_keys = image_keys + label_keys
    resizer = MonaiCropAndPadToShape3d_V2(out_spatial_dims, keys=combined_keys)
    dims = 3
    ...
    return Compose(synthetic_aug + [
        resizer,
        AffineAugment_V2(p=0.2, spatial_size=out_spatial_dims, dims=dims, axial_only=False, allow_translate=False, allow_shear=False, keys=combined_keys, mode=['bilinear' for _ in range(len(image_keys))] + ['nearest' for _ in range(len(label_keys))]),
        GaussianNoiseAugment(p=0.15, dims=dims, keys=image_keys),
        GaussianBlurAugment_V2(p=0.2, modality_p=0.5, dims=dims, keys=image_keys),
        BrightnessAugment(p=0.15, dims=dims, keys=image_keys),
        ContrastAugment(p=0.15, dims=dims, keys=image_keys),
        LowResolutionSimulationAugmentation_V2(p=0.25, modality_p=0.5, dims=dims, keys=image_keys),
...
def nnunet_augmentations_for_prog(image_keys=['FLAIR'], label_keys=['label', 'brainmask'], out_spatial_dims=(80, 192, 160), use_format_transform=True):
    combined_keys = image_keys + label_keys
    # resizer = MonaiCropAndPadToShape3d_V2(out_spatial_dims, keys=combined_keys)
    dims = 3
